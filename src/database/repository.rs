...
pub fn get_bookmark(conn: &SqliteConnection, input_key: &str) -> Result<Bookmark, diesel::result::Error> {
...
pub fn create_bookmark<'a>(conn: &SqliteConnection,
                           key: &'a str,
                           path: &'a str,
                           description: Option<&'a str>) -> Result<usize, diesel::result::Error> {
...
pub fn delete_bookmark(conn: &SqliteConnection, input_key: &str) -> Result<usize, diesel::result::Error> {
    use crate::schema::bookmarks::dsl::*;
    diesel::delete(bookmarks.filter(key.eq(input_key)))
        .execute(conn)
}

pub fn rename_bookmark(conn: &SqliteConnection, old_key: &str, new_key: &str) -> Result<usize, diesel::result::Error> {
    use crate::schema::bookmarks::dsl::*;
